...
/**
 * Main Home Page with Artist Profile Detection
 * 
 * ARTIST PROFILE CHECK:
 * - Detects if user already has an artist profile (user.artistProfile)
 * - Automatically redirects existing artists to Artist Studio
 * - Shows welcome message for new artists
 * - Handles loading states during profile detection
 * 
 * RESPONSIVE DESIGN:
 ...
 * 
 * USER FLOW:
 * - Authentication check → Profile detection → Appropriate redirect/display
 * - New users: Show main page with "Become an Artist" option
 * - Existing artists: Auto-redirect to Artist Studio
 * - Loading states: Proper spinners during checks
 */

const Home = () => {
  const navigate = useNavigate();
  const { showInfo } = useNotification();
  const { data: currentUser, isLoading: isUserLoading, error } = useGetUserQuery();

  // Check for existing artist profile and redirect if found
  useEffect(() => {
    if (!isUserLoading && currentUser) {
      ...
      if (currentUser.artistProfile) {
        ...
        // Small delay for better UX, then redirect to artist studio
        setTimeout(() => {
        ...
        <div className="flex-1 pl-4 pr-4 md:pl-6 md:pr-6 xl:pl-[22vw] xl:pr-[2vw] py-4 sm:py-6 transition-all duration-300">
          <div className="max-w-7xl mx-auto flex items-center justify-center min-h-[60vh]">
            <div className="max-w-md w-full bg-red-50/10 border border-red-200/20 rounded-2xl p-6 text-center">
              ...
              <p className="text-red-300 mb-4">Unable to load your profile information.</p>
              <button 
  ...
  // Show redirect message for existing artists
  if (currentUser?.artistProfile) {
    return (
      <div className="min-h-screen w-full flex flex-col">
        <div className="flex-1 pl-4 pr-4 md:pl-6 md:pr-6 xl:pl-[22vw] xl:pr-[2vw] py-4 sm:py-6 transition-all duration-300">
          <div className="max-w-7xl mx-auto flex items-center justify-center min-h-[60vh]">
            <div className="max-w-md w-full bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-2xl p-8 text-center">
              ...
              <h1 className="text-2xl font-bold text-white mb-4">Welcome Back, Artist!</h1>
              <p className="text-white/70 mb-6">
                You already have an artist profile. Redirecting you to your Artist Studio...
              </p>
  ...
  // Main home page for non-artists
  return (
    <div className="min-h-screen w-full flex flex-col">
      <div className="flex-1 pl-4 pr-4 md:pl-6 md:pr-6 xl:pl-[22vw] xl:pr-[2vw] py-4 sm:py-6 transition-all duration-300">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center py-12 sm:py-16 lg:py-20">
            ...
            <p className="text-xl sm:text-2xl text-white/70 mb-8 max-w-3xl mx-auto">
              Discover amazing music, connect with artists, and share your own creations with the world.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 py-12">
            {/* Discover Music */}
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8 text-center hover:bg-white/10 transition-all duration-300">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-white mb-3">Discover Music</h3>
              <p className="text-white/60 text-sm sm:text-base">
                Explore millions of tracks from artists around the world. Find your next favorite song.
              </p>
            </div>

            {/* Create Playlists */}
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8 text-center hover:bg-white/10 transition-all duration-300">
              <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 bg-gradient-to-br from-green-500/20 to-emerald-500/20 rounded-full flex items-center justify-center">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-white mb-3">Create Playlists</h3>
              <p className="text-white/60 text-sm sm:text-base">
                Organize your favorite tracks into playlists and share them with friends.
              </p>
            </div>

            {/* Share Your Music */}
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8 text-center hover:bg-white/10 transition-all duration-300">
              <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-full flex items-center justify-center">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" />
                </svg>
              </div>
              <h3 className="text-lg sm:text-xl font-semibold text-white mb-3">Share Your Music</h3>
              <p className="text-white/60 text-sm sm:text-base">
          ...
          {currentUser && (
            <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-2xl p-6 sm:p-8 text-center mt-12">
              ...
              <p className="text-white/70 text-base sm:text-lg mb-6 max-w-2xl mx-auto">
                Join thousands of artists who are already sharing their music and building their fanbase. 
                Create your artist profile today and start your musical journey.
              </p>
              <button
                onClick={() => navigate("/become-artist")}
